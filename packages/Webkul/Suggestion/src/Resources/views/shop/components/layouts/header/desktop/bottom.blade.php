    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-suggestion-searchbar-template">
            <div>
                ...
                <div
                    class="absolute z-[70] mt-2 w-full min-w-[445px] rounded-lg border border-zinc-200 bg-white shadow-xl"
                    id="suggest"
                ...
                >
                    ...
                    >
                        ...
                        @if(core()->getConfigData('suggestion.suggestion.general.show_searched_terms'))
                            <a
                                :href="'{{ route('shop.search.index') }}?query=' + encodeURIComponent(term) + '&sort=price-desc&limit=12&mode=grid'"
                                class="block border-b border-zinc-100 bg-white p-3 text-sm transition hover:bg-zinc-50"
                            >
                                <div>
        ...
        <script type="module">
            app.component('v-suggestion-searchbar', {
                ...
                watch: {
                    ...
                    term(newVal) {
                        if (newVal.length >= this.config.minSearchTerms) {
                            this.showDropdown = true;
                        } else {
                            this.showDropdown = false;
                        }
